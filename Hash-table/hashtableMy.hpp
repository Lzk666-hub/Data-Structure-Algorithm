...
protected:                                                                  // DO NOT USE private HERE!
    ...
    size_t findMinimumBucketSize(size_t bucketSize) const {
        ...
        auto floor=(size_t)((int)((double)tableSize/maxLoadFactor));
        auto nextSize= lower_bound(g_a_sizes,g_a_sizes+62,bucketSize);
        if(bucketSize>=g_a_sizes[61]){
            throw range_error("error");
        } else{
            while (*nextSize<=floor){
                nextSize++;
            }
        }
        return *nextSize;
    ...
    void updateBucketIt(){
        ...
        while (buckets.end()!=firstBucketIt&&firstBucketIt->empty()){
            firstBucketIt++;
        }
    }
...
public:
    ...
    Iterator find(const Key &key) {
        ...
        it.bucketIt=buckets.begin();
        for(size_t s=0;s< hashKey(key);s++){
            it.bucketIt++;
        }
        it.listItBefore=it.bucketIt->before_begin();
        if(!it.bucketIt->empty()){
            while (!it.endFlag&&it.bucketIt==buckets.begin()+ hashKey(key)){
    ...
    Iterator erase(const Iterator &it) {
        ...
        if(it.endFlag){
            ...
            Iterator temp = it;
            temp.listItBefore++;
            if (temp.listItBefore == temp.bucketIt->end()) {
                temp = it;
                return ++temp;
            }
            return it;
